...
ofstream log_file("CAS-Logfile.txt"); // Log file
...
int main()
{
    ...
    if (inp_file.is_open())
    {
        ...
        // Opening log file and writing logistics of CAS
        log_file << "CAS ME Output:" << endl;
        for (int i = 0; i < n; i++)
        {
            new_thread[i] = thread(testingCAS, i);
        }
        ...
        {
            cout << "Output file CAS-Logfile.txt failed to open" << endl;
        }

        // Opening stat file and writing stats of CAS
        ofstream stat_file;
        stat_file.open("CAS-Statfile.txt");
        ...
        {
            cout << "Output file CAS-Statfile.txt failed to open" << endl;
        }
        inp_file.close();
    }
    else
...
void testingCAS(int thread_id)
{
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        {
            int x = 0, y = 1;
            if (workingLock.compare_exchange_strong(x, y))
                break;
        }

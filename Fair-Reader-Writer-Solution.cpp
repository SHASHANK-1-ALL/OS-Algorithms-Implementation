...
ofstream log_file("Fair-RW-log.txt");   // log file to print logistics
...
void fair_rw_writer(int wtid);
void fair_rw_reader(int rtid);
...
int main()
{
    ...
    if (inp_file.is_open())
    {
        ...
        // converting mu_cs and mu_rem in seconds
        ...
        for (int i = 0; i < nw; i++)
        {
            writer_thread[i] = thread(fair_rw_writer, i);
        }
        for (int i = 0; i < nr; i++)
        {
            reader_thread[i] = thread(fair_rw_reader, i);
        }
        ...
        {
            cout << "Output file Fair-RW-log.txt failed to open" << endl;
        }

        // Opening stat file and writing statistics
        ofstream stat_file("Fair-RW-Avg_time.txt");
        ...
        {
            cout << "Output file Fair-RW-Avg_time.txt failed to open" << endl;
        }
        inp_file.close();
    }
    else
